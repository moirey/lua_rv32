...
local function dram_load(dram, addr, size)
  ...
  --print("dram_load: addr:"..tohex(addr-DRAM_BASE)..",size:"..size)
  local r_addr = ffi.new('uint32_t[1]',addr-DRAM_BASE)[0]

	return load_switch[size](dram,r_addr)
end
...
local function dram_store(dram, addr, size, value)
  ...
  --print("dram_store: addr:"..bit.tohex(addr-DRAM_BASE))
  local r_addr = ffi.new('uint32_t[1]',addr-DRAM_BASE)[0]
...
local function bus_load(bus, addr, size)
  ...
  elseif ( addr >= VIRTIO_BASE and addr < (VIRTIO_BASE+ VIRTIO_SIZE)) then 
    printf("can't process virtio_load")
    os.exit()
...
local function cpu_update_paging(cpu, csr_addr)
  if (csr_addr ~= CSR_REG.SATP) then return end
...
local exec_OP_S = {
  ...
  [OP_S.SW] = function(cpu, inst) 
     DBG("SW rs2:"..S_func[FUNC.rs2](inst)..",rs1:"..S_func[FUNC.rs1](inst)..",imm:"..S_func[FUNC.imm_I](inst).."\n")
...
local exec_OP_L = {
  [OP_L.LW] = function(cpu, inst) 
                DBG("LW rd:"..L_func[FUNC.rd](inst)..",rs1:"..L_func[FUNC.rs1](inst)..",imm:"..L_func[FUNC.imm_I](inst).."\n")
  ...
  [OP_L.LBU] = function(cpu, inst)
    ...
    cpu.regs[ L_func[FUNC.rd](inst) ] = cpu_load(cpu, cpu.regs[ L_func[FUNC.rs1](inst) ] + L_func[FUNC.imm_I](inst) ,8)
  end,
}
